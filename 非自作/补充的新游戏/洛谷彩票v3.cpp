...
int main(int argc, char *argv[]){
	...
	getchar();
	system("Cls") ;
	cout<<"欢迎来到洛谷彩票！"<<endl<<endl<<"温馨提示：在游戏内请勿使用小数、负数、英文字母、特殊字符（游戏中另说明除外），"<<endl<<"否则后果自负。"<<endl<<endl;
    ...
    if(c[0]=='b'||c[0]=='B'){
        while(money>0)
        {
            ...
            for(int i=0; i<n; i++)
            {
                p=(rand()%12000)*floor((1+(lucky/100)));
                if(p==0)
                {
                    int q=0;
                    q=rand()%4;
                    if(q==1){
                        cout<<"你获得了杰克壶！"<<endl;
                        if (d==5){
						money+=5000;lucky+=500;
						}
                        else money+=10000;lucky+=1000;
                    }
                }
                else if(p>=1&&p<=8)
                {
